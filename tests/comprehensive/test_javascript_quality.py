        ...
        for js_file in JS_BASE_DIR.rglob("*.js"):
            ...
            for i, line in enumerate(lines, 1):
                ...
                if len(consts) != len(set(consts)):
                    issues.append(
                        f"{js_file.relative_to(JS_BASE_DIR)}:{i} - Duplicate const declaration in same line"
                    )
        ...
        for js_file in JS_BASE_DIR.rglob("*.js"):
            ...
            for i, line in enumerate(lines, 1):
                ...
                if re.search(r"\bvar\s+\w+", line):
                    issues.append(
                        f"{js_file.relative_to(JS_BASE_DIR)}:{i} - Uses 'var' instead of const/let"
                    )
    ...
    def test_no_duplicate_element_ids_in_templates(self):
        ...
        for js_file in JS_BASE_DIR.rglob("*.js"):
            ...
            for template in templates:
                ...
                if duplicates:
                    issues.append(
                        f"{js_file.relative_to(JS_BASE_DIR)} - Duplicate IDs: {', '.join(duplicates)}"
                    )
        ...
        for js_file in JS_BASE_DIR.rglob("*.js"):
            ...
            for i, line in enumerate(lines, 1):
                ...
                if any(
                    ...
                    for kw in ["const ", "let ", "var ", "return "]
                ):
                    if not stripped.endswith(";") and not stripped.endswith(","):
                        issues.append(
                            f"{js_file.relative_to(JS_BASE_DIR)}:{i} - Statement should end with semicolon"
                        )
...
class TestJavaScriptCriticalIssues:
    ...
    def test_no_duplicate_settings_modal_sections(self):
        ...
        assert (
            analysis_settings_count <= 1
        ), f"Found {analysis_settings_count} 'Analysis Settings' sections (should be 1)"
        assert (
            monte_carlo_count <= 1
        ), f"Found {monte_carlo_count} 'Monte Carlo Analysis' sections (should be 1)"
