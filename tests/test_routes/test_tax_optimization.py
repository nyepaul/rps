...
def test_get_tax_snapshot(client, test_user, test_profile):
    ...
    response = client.post('/api/tax-optimization/snapshot', json={
...
def test_analyze_comprehensive(client, test_user, test_profile):
    ...
    response = client.post('/api/tax-optimization/analyze', json={
        'profile_name': 'Test Profile',
        'filing_status': 'mfj',
        'state': 'CA'
    })
...
def test_analyze_roth_conversion(client, test_user, test_profile):
    ...
    response = client.post('/api/tax-optimization/roth-conversion', json={
        'profile_name': 'Test Profile',
        'filing_status': 'mfj'
    })
...
def test_analyze_social_security(client, test_user, test_profile):
    ...
    response = client.post('/api/tax-optimization/social-security-timing', json={
        'profile_name': 'Test Profile',
        'life_expectancy': 90
    })
...
def test_state_comparison(client, test_user, test_profile):
    ...
    response = client.post('/api/tax-optimization/state-comparison', json={
        'profile_name': 'Test Profile'
    })
...
def test_rmd_projection(client, test_user, test_profile):
    ...
    response = client.post('/api/tax-optimization/rmd-projection', json={
        'profile_name': 'Test Profile',
        'growth_rate': 0.05,
        'years': 20
    })
