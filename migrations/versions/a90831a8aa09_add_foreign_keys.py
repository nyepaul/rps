...
def upgrade() -> None:
    ...
    # Step 1: Recreate profiles table with user_id and foreign key
    op.execute('DROP TABLE IF EXISTS profiles_new')
    op.execute('''
    ...
    # Copy existing data (match profiles by profile_name)
    op.execute('''
        ...
        FROM action_items a
        LEFT JOIN profiles p ON p.name = a.profile_name AND p.user_id = 1
        WHERE EXISTS (SELECT 1 FROM users WHERE id = 1)
    ''')
    ...
    # Copy existing data (match profiles by profile_name)
    op.execute('''
        INSERT INTO conversations_new (id, user_id, profile_id, role, content, created_at, updated_at)
        SELECT c.id, 1, p.id, c.role, c.content, c.created_at, CURRENT_TIMESTAMP
        FROM conversations c
        LEFT JOIN profiles p ON p.name = c.profile_name AND p.user_id = 1
        WHERE EXISTS (SELECT 1 FROM users WHERE id = 1)
    ''')
...
def downgrade() -> None:
    ...
    op.execute('''
        INSERT INTO profiles_new (id, name, birth_date, retirement_date, data, updated_at)
