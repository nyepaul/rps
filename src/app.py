...
from src.routes.home_ownership import home_ownership_bp
...
def create_app(config_name="development"):
    ...
    def set_security_headers(response):
        ...
        response.headers["X-Content-Length"] = response.content_length or len(
            response.get_data()
        )
        ...
        response.headers["X-Frame-Options"] = (
            "SAMEORIGIN"  # Changed from DENY to SAMEORIGIN for iframe support
        )
    ...
    def verify_email_page():
        return send_from_directory(app.static_folder, "verify-email.html")
