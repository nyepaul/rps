...
class Profile:
    ...
    def save(self):
        ...
        with connection.db.get_connection() as conn:
            ...
            if is_new:
                ...
                # Log creation
                log_create(
                    "profiles", self.id, self.user_id, f"Created profile: {self.name}"
                )
            else:
                ...
                # Log update
                log_update(
                    "profiles", self.id, self.user_id, f"Updated profile: {self.name}"
                )
        return self
    ...
    def delete(self):
        ...
        if self.id:
            ...
            log_delete(
                "profiles", self.id, self.user_id, f"Deleted profile: {self.name}"
            )
            with connection.db.get_connection() as conn:
                conn.execute(
