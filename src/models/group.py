...
class Group:
    ...
    def add_member(self, user_id: int):
        ...
        with db.get_connection() as conn:
            conn.execute('INSERT OR IGNORE INTO user_groups (user_id, group_id) VALUES (?, ?)', (user_id, self.id))

    def remove_member(self, user_id: int):
        """Remove a user from this group."""
        with db.get_connection() as conn:
            conn.execute('DELETE FROM user_groups WHERE user_id = ? AND group_id = ?', (user_id, self.id))
