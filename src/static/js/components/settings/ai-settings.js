...
function validateApiKey(provider, value) {
    ...
    switch (provider) {
        case 'gemini':
            if (!value.startsWith('REDACTED_GAPI_')) {
                return { valid: false, error: 'Gemini keys must start with "REDACTED_GAPI_"' };
            }
            if (value.length !== 39) {
            ...
            break;
        case 'claude':
            if (!value.startsWith('REDACTED_ANTH_')) {
                return { valid: false, error: 'Claude keys must start with "REDACTED_ANTH_"' };
            }
            break;
        case 'openai':
            if (!value.startsWith('sk-') && !value.startsWith('REDACTED_OPENAI_')) {
                return { valid: false, error: 'OpenAI keys must start with "sk-" or "REDACTED_OPENAI_"' };
            }
            break;
        case 'grok':
            ...
            break;
        case 'openrouter':
            if (!value.startsWith('REDACTED_OR_')) {
                return { valid: false, error: 'OpenRouter keys must start with "REDACTED_OR_"' };
            }
            break;
        case 'huggingface':
